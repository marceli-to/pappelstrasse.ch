...
class GetData
{
  ...
  { 
    ...
    // filter out collective listings (Sammelinserate) with ref_house 99
    $data = $data->filter(function ($apartment) {
      return ($apartment['ref_house'] ?? '') !== '99';
    });
  ...
  private function getState($all_listings)
  {
    ...
    // first we handle all removed listings and set the status to taken
  ...
  private function handleRemovedListings($all_listings, &$listing_status)
  {
    ...
    */
    foreach ($all_listings as $listing) {
      ...
      $listing_status[$listing_ref] = $this->status_taken;
    }
    return $listing_status;
  }
